# Script for creating moisture maps from NDRE model predictions
...
raster_soilM_plots <- transform(raster_soilM_plots,
                                cutoff_soilM = ifelse(ndre_val < 0.45, NA, soilM))
...
Wgg <- ggplot() +
    geom_raster(data = W_r, aes(x = x, y = y, fill = cutoff_soilM),
                alpha = 0.95, interpolate = T) +
...
AESgg <- ggplot() +
    geom_raster(data = AES_r, aes(x = x, y = y, fill = cutoff_soilM),
                alpha = 0.95, interpolate = T) +
    scale_fill_viridis(direction = -1, begin = 0.05, end = 0.85,
                       limits = c(0.14, 0.25)) +
    coord_equal() +
...
ODgg <- ggplot() +
    geom_raster(data = OD_r, aes(x = x, y = y, fill = cutoff_soilM),
                alpha = 0.95, interpolate = T) +
    scale_fill_viridis(direction = -1, begin = 0.05, end = 0.85,
                       limits = c(0.14, 0.25)) +
    coord_equal() +
...
Jgg <- ggplot() +
    geom_raster(data = J_r, aes(x = x, y = y, fill = cutoff_soilM),
                alpha = 0.95, interpolate = T) +
    scale_fill_viridis(direction = -1, begin = 0.05, end = 0.85,
                       limits = c(0.14, 0.25)) +
    coord_equal() +
