...
# weather_cleaned <- read.csv(file = './HydroSatML/data/weather/weather_data_cleaned.csv')

coords <- read.csv('./data/SCF_TierII_site_coords.csv')
...
merge_df <- data.frame('orig_date' = ndre_soilM_weather_ssc_crop_join$date,
                       'field' = ndre_soilM_weather_ssc_crop_join$field,
                       'sensor' = ndre_soilM_weather_ssc_crop_join$sensor)
date_vec <- ndre_soilM_weather_ssc_crop_join$date

lagged_soilM_df <- function(data, soilMdata, merge_df, lag=c(3,5,7)){
    tmp_list <- list()
...
weather <- transform(weather, date = as.Date(as.character(date), format = '%m/%d/%Y'),
                     field = factor(field))
####
# crop stuff
####
...
for(i in 1:nrow(joined_data)){
    if(all(is.na(joined_data[i,c(9:11)]))){
        idx[t] <- i
        t = t + 1
    }
}
...
write_csv(x = joined_data_no_bare_soil, path = './HydroSatML/data/final_join_subbed_bare_soil_with_lag_40.csv')
